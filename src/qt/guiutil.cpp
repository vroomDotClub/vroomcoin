...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("vroomcoin"))
        return false;
...
{
    // Convert vroomcoin:// to vroomcoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("vroomcoin://"))
    {
        uri.replace(0, 12, "vroomcoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Vroomcoin.lnk";
}
...
{
    return GetAutostartDir() / "vroomcoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Vroomcoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  vroomcoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("Vroomcoin-Qt"));
